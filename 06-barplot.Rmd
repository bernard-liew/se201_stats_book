...
There are so many types of figures that you can create in R (Figure \@ref(fig:FIG-RICH-GRAPH)). I will only delve into one type of figure that I believe is most useful for your case. With this basic skills, you have the foundation to delve deeper into this software if you desire. 
...
Let us prepare for this chapter by importing the necessary datasets of `groupFMS.xlsx`. 
...
```
Group FMS summary table

Let's create a summary table with three columns, `task`, `score`, and `Count`. This could should be what you have gotten from learning check \@ref(WRANGLE-LEARNING-CHECK).

```{r}
dat_grp <- dat_fms %>% # original data
...
Let's extract the row containing athlete c's data, and make it long.

```{r}
athlete_c <- dat_fms %>%
  filter (id == "athlete_c") %>% # original data
  pivot_longer(cols = -id,
              names_to = "task",
              values_to = "score")%>% 
  mutate (
...
Let us also create one more dataset using `athlete_c` data, and take the lower of the two scores for tasks which are assessed bilaterally. 
...
In this example we'll use the `athlete_c` data set, in which we have an FMS score one for each `side`. 
...
```
Let's try this example on another dataset `dat_grp`, in which we have the number of subjects who attain a specific FMS `score` for each `task`.  
...
1. Open up your `practice_script.R`, updated from the learning check in \@ref(WRANGLE-LEARNING-CHECK).You should already have codes to help you import your files, and process your files.

2. Using the data `athlete_c_long` data, create a barplot athlete C's FMS score. Make the variable `task` as the *x* axis, the `score` FMS score as the *y* axis, and the `side` as the `fill` color. See Recipe \@ref(BAR-GRAPH-GROUPED-BAR).

4. Using the data `fms_count` data, make `score` a factor. See Recipe \@ref(WRANGLE-CHARACTERS-2-FACTORS).

5. Using the data `fms_count` data, create a barplot, where the variable `task` is on the *x* axis, the `count` is on the *y* axis, and  FMS `score` as the `fill` color. See Recipe \@ref(BAR-GRAPH-GROUPED-BAR).
