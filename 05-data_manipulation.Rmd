# Data manipulation - part 1 {#WRANGLE}
...
Typically a dataset will only suffer from one of these problems; it'll only suffer from both if you're really unlucky! To fix these problems, you'll need the two most important functions in tidyr: `pivot_wider()` in Recipe \@ref(WRANGLE2-SPREAD) and `pivot_longer()` in Recipe \@ref(WRANGLE2-GATHER) below.


## Renaming variables {#WRANGLE-RENAME-VARIABLES}

```{r, echo = FALSE}
vembedr::embed_youtube("JB4Ex9oFZi0", height = 400)
```
...
```{r, echo = FALSE}
vembedr::embed_youtube("v55dRA291uo", height = 400)
```

### Problem

You want to remove a specific row  or a range of rows. See also Recipe \@ref(WRANGLE2-FILTER).
...
```{r, echo = FALSE}
vembedr::embed_youtube("qmz7c_HPUEQ", height = 400)
```
...
```{r, echo = FALSE}
vembedr::embed_youtube("j1J63-ePZd8", height = 400)
```
...
```{r, echo = FALSE}
vembedr::embed_youtube("rBIq-n-RZxI", height = 400)
```
...
Use the `mutate()` function and the `as.numeric` function. You will learn the `mutate ()` function more in Recipe \@ref(WRANGLE2-NEW-VARIABLES).
...
```{r, echo = FALSE}
vembedr::embed_youtube("imDxCPMgXfE", height = 400)
```
