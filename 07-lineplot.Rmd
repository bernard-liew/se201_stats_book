...
Let us prepare for this chapter by first importing a Vo2 treadmill test data that we have cleaned, and placed in the data folder. See Recipe \@ref(WRANGLE2-SUMMARIZE-GROUPBY). The Excel sheet is called `"treadmill_pt4_clean.xlsx"`. 
...
```{r, echo = FALSE}
vembedr::embed_youtube("m0rdWRnWgc8", height = 400)
```
...
```{r, echo = FALSE}
vembedr::embed_youtube("VlyE4ZB0RPM", height = 400)
...
To convert a variable to a factor , see also Recipe \@ref(WRANGLE2-CHARACTERS-2-FACTORS).
...
```{r, echo = FALSE}
vembedr::embed_youtube("xOWAMHWUxC4", height = 400)
```
...
The type of line (solid, dashed, dotted, etc.) is set with `linetype`, the thickness (in mm) with `linewidth`, and the color of the line with `colour` (or `color`).
...
```{r FIG-BAR-GRAPH-BASIC-LINE-CUSTOMIZED, fig.cap="Line graph with custom linetype, linewidth, and colour"}
ggplot(dat) +
  geom_line(aes(x = stage, y = vo2), 
            linetype = "dashed", linewidth = 1, colour = "blue")
```
...
To set a single constant color for all the lines, specify colour outside of `aes()`. The same works for linewidth, linetype, and point shape (Figure \@ref(fig:FIG-LINE-GRAPH-MULTI-LINE-CONSTANT-AES)). You may also have to specify the grouping variable:


(ref:cap-FIG-LINE-GRAPH-MULTI-LINE-CONSTANT-AES) Line graph with constant linewidth and color (left); With `supp` mapped to `colour`, and with points added (right)
...
ggplot(dat_long) +
  ...
  geom_point(aes(x = stage, y = val, colour = var),shape = 22, linewidth = 3, fill = "white")
...
```{r, echo = FALSE}
vembedr::embed_youtube("IpmG8N8u6lw", height = 400)
```
...
```{r, echo = FALSE}
vembedr::embed_youtube("7M5_GRm6bYM", height = 400)
```
...
7. For each stage, remove the data between 3 to 3:30min. See Recipe \@ref(WRANGLE2-SUMMARIZE-GROUPBY) and \@ref(WRANGLE2-FILTER-KEEP-NUMERIC).

8. For each stage, keep the last six rows of data. See Recipe \@ref(WRANGLE2-SUMMARIZE-GROUPBY) and \@ref(WRANGLE-SLICE).

9. For each stage, calculate for all variables for the mean of the last six rows of data. See Recipe \@ref(WRANGLE2-SUMMARIZE-GROUPBY) and \@ref(WRANGLE2-SUMMARIZE-GLOBAL). Assign this table to an object called `dat_summ`.

10. Merge the two tables `dat_summ` and `dat_stage`, by the common variable called `stage`. See Recipe \@ref(WRANGLE2-INNER-JOIN). Assign this table to an object called `df_plot`.
